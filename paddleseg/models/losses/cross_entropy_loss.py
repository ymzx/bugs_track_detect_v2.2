...
class CrossEntropyLoss(nn.Layer):
    ...
    def __init__(self,
        ...
        if weight is not None: weight = paddle.to_tensor(weight, dtype='float32')
        self.weight = weight
    ...
    def forward(self, logit, label, semantic_weights=None):
        ...
        if self.weight is not None and logit.shape[channel_axis] != len(self.weight):
            raise ValueError(
                'The number of weights = {} must be the same as the number of classes = {}.'
                .format(len(self.weight), logit.shape[1]))

        if channel_axis == 1: logit = paddle.transpose(logit, [0, 2, 3, 1])
        ...
        loss = loss * mask
        if semantic_weights is not None: loss = loss * semantic_weights
